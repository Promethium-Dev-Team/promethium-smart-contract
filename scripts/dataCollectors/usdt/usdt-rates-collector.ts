...
const path = "./arbitrumUSDTAugust2022.csv";

let rates: number[] = [0, 0, 0, 0, 0];
let startBlockNumber = 19075859;
let finishBlockNumber = 22253578;
...
async function main() {
    ...
    for (
        ...
        rates[5] = 0;
        // BigNumber.from(await getRadiantV2Rate(currentBlockNumber))
        //     .div(1e6)
        //     .toNumber() / 1e12;

        rates[6] =
            BigNumber.from(await getDForceRate(currentBlockNumber))
                .div(1e6)
                .toNumber() / 1e12;
