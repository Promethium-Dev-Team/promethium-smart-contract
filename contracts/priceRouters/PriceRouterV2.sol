...
contract PriceRouterV2 is Initializable, IPriceRouter, UUPSUpgradeable, OwnableUpgradeable {
    address public usdt;
    ...
    function getTokenValue(address token, address itoken, uint256 amount) external view returns (uint256) {
        if (token == usdt) {
            ...
            //radiant V2
            if (itoken == address(0x0D914606f3424804FA1BbBE56CCC3416733acEC6)) {
                return amount;
            }
            //AAVE V3
            if (itoken == address(0x82E64f49Ed5EC1bC6e43DAD4FC8Af9bb3A2312EE)) {
                return amount;
            }
            //granary
            if (itoken == address(0xFC2eaC1AeB490d5ff727E659273C8AfC5dD2b0bb)) {
                return amount;
            }
            //lodestar
            if (itoken == address(0x4987782da9a63bC3ABace48648B15546D821c720)) {
                return ((amount * iTokenLodestar(itoken).exchangeRateStored()) / (10 ** 18));
            }
            //wepiggy
            if (itoken == address(0xDe39Adfb2025D2aA51f6fD967e7C1753215f1905)) {
                return ((amount * iTokenWePiggy(itoken).exchangeRateStored()) / (10 ** 18));
            }
            //dForce
            if (itoken == address(0xf6995955e4B0E5b287693c221f456951D612b628)) {
                return ((amount * iTokenDForce(itoken).exchangeRateStored()) / (10 ** 18));
            }
            //tender
            if (itoken == address(0xB287180147EF1A97cbfb07e2F1788B75df2f6299)) {
                return ((amount * iTokenTender(itoken).exchangeRateStored()) / (10 ** 18));
            }
        } else if (token == frax) {
            //lodestar
            if (itoken == address(0xD12d43Cdf498e377D3bfa2c6217f05B466E14228)) {
                return ((amount * iTokenLodestar(itoken).exchangeRateStored()) / (10 ** 18));
            }
            //dForce
            if (itoken == address(0xb3ab7148cCCAf66686AD6C1bE24D83e58E6a504e)) {
                return ((amount * iTokenDForce(itoken).exchangeRateStored()) / (10 ** 18));
            }
            //tender
            if (itoken == address(0x27846A0f11EDC3D59EA227bAeBdFa1330a69B9ab)) {
                return ((amount * iTokenTender(itoken).exchangeRateStored()) / (10 ** 18));
            }
        }
    ...
    function initialize(address _usdt, address _usdc_e, address _wbtc, address _weth, address _arb)
        external
        initializer
    {
